...
class DBClient():
    ...
    def __init__(self, database, host='localhost', port='5432', user='postgres', password='1234') -> None:
    ...
    def _get_engine(self, user, password):
        engine = create_engine(f'postgresql+psycopg2://{user}:{password}@{self.host}:{self.port}/{self.database}')
        return engine
    ...
    def write(self, df, table_name, conn, chunksize=5000, if_exists="replace", method='multi', index=False):
        ...
        df.to_sql(name=table_name, con=conn, chunksize=chunksize, method=method, index=index, if_exists=if_exists)
